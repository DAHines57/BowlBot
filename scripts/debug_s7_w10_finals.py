"""Debug Season 7 week 10: Rolling Stoned vs Spare Parts."""
...
s7w10 = [f for f in facts if f.get("season_number") == 7 and f.get("week") == 10]
teams = ["Rolling Stoned", "Spare Parts"]
for t in teams:
    rows = [f for f in s7w10 if f.get("team") == t and not f.get("substitute")]
...
md = get_week_matchups(facts, 10, season="Season 7", season_num=7)
for m in md.get("matchups", []):
    h, a = m["home"], m.get("away")
    if not a:
        continue
    names = {h["name"], a["name"]}
    if "Rolling" in str(names) or "Spare" in str(names):
        print(
            "MATCHUP:",
            h["name"],
            h["wins"],
